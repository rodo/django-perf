...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        objs = Apple.objects.filter(indice=indice)

        for obj in objs:
            obj.keyid = obj.indice
            obj.save(update_fields=['keyid'])

        log.stop = datetime.now()
        log.save()

        print test, log.stop - log.start
        print "indice %d -> %d items" % (indice, objs.count())
